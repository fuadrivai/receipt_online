...
class DailyTaskDetail extends StatefulWidget {
  ...
  const DailyTaskDetail({super.key, required this.dailyTask});
...
class _DailyTaskDetailState extends State<DailyTaskDetail> {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        children: List.generate((widget.dailyTask.receipts ?? []).length, (i) {
          Receipt e = (widget.dailyTask.receipts ?? [])[i];
          return CardBanner(
            title: e.number ?? "--",
            subtitle:
                Jiffy.parseFromDateTime(DateTime.parse(e.createdAt!).toLocal())
                    .yMMMdjm,
            leading: Padding(
